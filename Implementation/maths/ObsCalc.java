...
public class ObsCalc {
	...
	public static boolean pointWithin(Point2D point, Renderable shape) {
		if((numberOfIntersects(shape, new Line2D.Double(point,new Point2D.Double(700,700))) & 1)==0){
			return false;
		}
		return true;
		//		RenderableClass rClass = RenderableClass.valueOf(shape.getClass()
	...
	public static Point2D[] intersectingPoints(Renderable shape,
			Line2D line) {
		RenderableClass rClass = RenderableClass.valueOf(shape.getClass()
				.getSimpleName());
		switch (rClass) {
	...
	private static boolean doesCross(RenderableOval rOval,
			Line2D line) {
		if (pointWithin(getClosestPointOnLine(line, new Point2D.Double(rOval.centreX,rOval.centreY)), rOval)) {
			return true;
		}
		return false;
	}

	public static int numberOfIntersects(Renderable shape,
			Line2D line) {
		Point2D[] intersects =intersectingPoints(shape,line);
		if(intersects==null){
			return 0;
